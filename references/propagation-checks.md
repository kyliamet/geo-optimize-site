...
- If an intended source still exposes the superseded fact, report `source-not-fixed` with the affected URLs and evidence. This takes precedence over other source-readiness states. Stop propagation classification until the source is corrected; run a downstream query only if the user separately requests a time-stamped observation rather than a propagation conclusion.
- If no checked source exposes the superseded fact but access, authentication, rendering, or conflicting evidence prevents a defensible conclusion about the origin or intended canonical content, report `source-unverified` with the constraint. Do not use this state merely because a sitemap, feed, or other crawl signal is unavailable. Do not classify propagation; run a downstream query only if the user separately requests a time-stamped observation.
...
- Report the first matching source-readiness state for the overall check:
  ...
  - `source-unverified`: no checked source exposes the superseded fact, but available evidence cannot establish whether the origin or intended canonical content is corrected.
  - `site-fixed`: the origin is verified as corrected and no checked source exposes the superseded fact, but one or more intended crawl signals are stale or could not be verified as updated.
  - `crawl-signals-updated`: the origin and intended discovery signals are verified as corrected.
- Use `propagated` only when source readiness is `crawl-signals-updated`. For any earlier source-readiness state, separately requested downstream queries may use only `current-observation`, `engine-stale`, or `unable-to-verify`.
